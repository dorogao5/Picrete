...
fn build_system_prompt(prompt: &str, profile: &str, reference: &str) -> String {
    format!(
        "{prompt}\n\n{profile}\n\nКОНТЕКСТ СЕССИИ\n\
         ...
         Если определение или сравнение составляет задание, не перечисляйте весь набор определений: объясните одно базовое понятие и предложите применить его к одному случаю. Вопрос после перечисления ответов не делает помощь пошаговой. Не маскируйте выдачу решения словами «не дам эталон, но». Подтверждайте верные шаги, которые студент уже показал. Полный разбор доступен отдельным действием платформы, просьба в чате его не включает. \
